        ...
        <div class="white_card mb_30 shadow pt-4">
            <div class="white_card_body">
                <div class="QA_section">
                    <div>
                        <form action="{{route('mrp.place-update',$place->id)}}" method="post">
                            @method('patch')
                            @csrf
                            <div class="form-group">
                                <label for="">Place Code</label>
                                <input type="text" value="{{$place->place_code}}"
                                    class="form-control @error('code') is-invalid @enderror" name="place_code">
                                @error('place_code')
                                ...
                                <input type="text" value="{{$place->place_name}}"
                                    class="form-control @error('name') is-invalid @enderror" name="place_name">
                                @error('place_name')
                                ...
                                <textarea 
                                    class="form-control @error('description') is-invalid @enderror" name="description">{{$place->description}}</textarea>
                                @error('description')
