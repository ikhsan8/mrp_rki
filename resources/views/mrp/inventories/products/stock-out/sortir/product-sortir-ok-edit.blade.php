        ...
        <div class="white_card mb_30 shadow pt-4">
            <div class="white_card_body">
                <div class="QA_section">
                    <div>
                        <form action="{{route('mrp.product-sortir-update-sortir-ok',$product_sortir_ok->id)}}" method="post">
                            @method('patch')
                            @csrf
                                ...
                                <select class="form-control @error('inventory_product_list_id') is-invalid @enderror" name="inventory_product_list_id" id="inventory_product_list_id">
                                    ...
                                    @foreach ($inven_products as $product)
                                        <option value="{{ $product->id }}" 
                                            {{ ((old('inventory_product_list_id') ?? $product_sortir_ok->inventoryproductList->product->id) == $product->product->id) ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" min="0" value="{{$product_sortir_ok->qty_ok}}" id="qty_ok" onkeyup="changeStock()"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" value="{{ $product_sortir_ok->date }}" class="form-control digits minMaxExample
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('employee_id') is-invalid @enderror" name="employee_id">
                                    ...
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ ((old('employee_id') ?? $product_sortir_ok->employee->id) == $employee->id) ? 'selected' : '' }}>{{ $employee->employee_name }}</option>
                                    @endforeach
                                </select>
                                @error('employee_id')
                                    <span class="invalid-feedback" role="alert">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" value="{{ $product_sortir_ok->current_stock }}" class="form-control  
                                ...
                                <textarea 
                                    class="form-control @error('description') is-invalid @enderror" name="description">{{$product_sortir_ok->description}}</textarea>
                                @error('description')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
