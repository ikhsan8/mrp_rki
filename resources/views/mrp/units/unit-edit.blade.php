        ...
        <div class="white_card mb_30 shadow pt-4">
            <div class="white_card_body">
                <div class="QA_section">
                    <div>
                        <form action="{{route('mrp.unit-update',$unit->id)}}" method="post">
                            @method('patch')
                            @csrf
                            <div class="form-group">
                                <label for="">Unit Code</label>
                                <input type="text" value="{{$unit->unit_code}}"
                                ...
                                <input type="text" value="{{$unit->unit_name}}"
                                ...
                                <textarea 
                                    class="form-control @error('description') is-invalid @enderror" name="description">{{$unit->description}}</textarea>
                                @error('description')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
