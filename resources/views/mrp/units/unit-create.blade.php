        ...
        <div class="white_card mb_30 shadow pt-4">
            <div class="white_card_body">
                <div class="QA_section">
                    <div>
                        <form action="{{route('mrp.unit-store')}}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="">Unit Code</label>
                                <input type="text" value="{{old('unit_code')}}"
                                    class="form-control @error('unit_code') is-invalid @enderror" name="unit_code">
                                @error('unit_code')
                                <span class="text-danger">*Unit Code Wajib Diisi!</span>
                                @enderror
                                <br>
                                <label for="">Unit Name</label>
                                <input type="text" value="{{old('unit_name')}}"
                                    class="form-control @error('unit_name') is-invalid @enderror" name="unit_name">
                                @error('unit_name')
                                <span class="text-danger">*Unit Name Wajib Diisi!</span>
                                @enderror
                                <br>
