                    ...
                    <div class="QA_section">
                        <div class="white_box_tittle list_header">
                            ...
                            <div class="box_right d-flex lms_block">
                                ...
                                <a href="{{ route('mrp.delivery.delivery_shipment.delivery_shipment-create') }}">
                        ...
                        <div class="QA_table mb_30">
                            <!-- table-responsive -->
                            <table class="table delivery_shipment ">
                                <thead>
                                    <tr>
                                        <th scope="col">No</th>
                                        <th scope="col">DN Code</th>
                                ...
                                <tbody>
                                    @foreach ($shipments as $shipment)
                                        <tr>
                                            ...
                                            <td>{{ $shipment->customer->customer_name ?? 'N/A' }}</td>
                                            <td>{{ $shipment->vehicle->car_code }}</td>
                                            <td>
                                                <div class="action_btns d-flex">
                                                    @if (auth()->user()->can('delivery_shipment-edit'))
                                                    <a href="{{ route('mrp.delivery.delivery_shipment.delivery_shipment-edit', $shipment->id) }}"
                                                        class="action_btn mr_10"> <i class="far fa-edit"></i> </a>
                                                    @endif
                                                    @if (auth()->user()->can('delivery_shipment-delete'))
                                                    <a href=""
                                                        onclick="deleteData(event,{{ $shipment->id }},'{{ $shipment->dn_code }}')"
...
<script src="{{asset('assets')}}/vendors/datatable/js/buttons.print.min.js"></script>
    <script>
        if ($('.delivery_shipment').length) {
            $('.delivery_shipment').DataTable({
                bLengthChange: false,
                "bDestroy": false,
                language: {
        ...
        var urlDelete = `{{ route('mrp.delivery.delivery_shipment.delivery_shipment-delete') }}`
