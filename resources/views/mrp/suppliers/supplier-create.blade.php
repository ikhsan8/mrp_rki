        ...
        <div class="white_card mb_30 shadow pt-4">
            <div class="white_card_body">
                <div class="QA_section">
                    <div>
                        <form action="{{route('mrp.supplier-store')}}" method="post">
                            @csrf
                            <div class="form-group">
                                <label for="">Supplier Code</label>
                                <input type="text" value="{{old('supplier_code')}}"
                                    class="form-control @error('supplier_code') is-invalid @enderror" name="supplier_code">
                                @error('supplier_code')
                                ...
                                <input type="text" value="{{old('supplier_name')}}"
                                    class="form-control @error('supplier_name') is-invalid @enderror" name="supplier_name">
                                @error('supplier_name')
                                ...
                                @error('address')
                                <span class="text-danger">*Addres Wajib Diisi!</span>
                                @enderror
                                <br>
                                <label for="">Phone</label>
                                <input type="number" min="0" value="{{old('phone')}}"
                                ...
                                <textarea 
                                    class="form-control @error('email') is-invalid @enderror" name="email">{{old('email')}}</textarea>
